...
import { AI_MODEL_CONSOLIDATION } from "./config.js";
import { MEMORY_EXCLUSION_NOTE, memoryExclusionReasons } from "./memoryFilter.js";
...
const CONSOLIDATION_MAX_TOKENS = 500;  // token budget for reflection completion
const CONSOLIDATION_TEMPERATURE = 0.4; // moderate temp for reflective prose
...
async function reflect({ sessions, pinned, unpinned }) {
  // Filter the ground truth on READ as well as on write. Rows written before
  // the exclusion filter existed are still in the table until the purge runs,
  // and consolidation is exactly the path that would launder one of them into
  // a durable core memory.
  const log = sessions
    .slice()
    .reverse() // chronological for the model
    .filter((s) => memoryExclusionReasons(s.summary).length === 0)
  ...
  const system =
    ...
    "\n\n" + MEMORY_EXCLUSION_NOTE.replace(
      'return {"summary": "", "frame_tag": "routine"}',
      "return one fewer item"
    );

  const userMsg = `SESSION LOG (oldest to newest):\n${log}${pinnedBlock}${continuityBlock}`;

  const data = await openrouterChat(AI_MODEL_CONSOLIDATION,
    [
  ...
  try {
    ...
    return parsed
      ...
      // Enforcement half of the exclusion filter (Addendum C1/C5). Core
      // memories are the DURABLE tier — a governed fact that leaks in here
      // outlives every session memory it was derived from — so an item that
      // trips a rule is dropped and the set simply comes back one shorter.
      // why_it_carries is checked too: "matters because they're 14" is the
      // same leak wearing a different key.
      .filter((d) => {
        const reasons = memoryExclusionReasons(
          d.text + " " + (d.why_it_carries || "")
        );
        if (reasons.length > 0) {
          console.warn("core memory rejected by exclusion filter:", JSON.stringify({ reasons }));
          return false;
        }
        return true;
      });
