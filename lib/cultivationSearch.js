...
export function retrieveCultivation(message) {
  ...
  const topN = ranked.slice(0, 6).map((r) => r.id);

  // Best cluster = most members present in the top matches (needs >= 2).
  let bestKey = null, bestCount = 1, bestMembers = null;
  for (const [key, members] of Object.entries(clustersCache)) {
    const n = members.filter((m) => topN.includes(m)).length;
    if (n > bestCount) { bestCount = n; bestKey = key; bestMembers = members; }
  }

  let memberIds, leadId;
  if (bestKey) {
    memberIds = bestMembers;
    // Lead from the CURATED cluster order (authoring priority), not the noisy
    // matcher rank. Reassurance-first only when a normal/healthy explanation is
    // an EARLY (top-2) cluster member — true for white-stuff → trichome-frost,
    // but not yellowing → late-fade, so a mid-grow feeding question isn't
    // waved off as "just normal fade."
    const earlyNormal = memberIds.slice(0, 2).find((m) => issueById.get(m)?.is_normal);
    leadId = earlyNormal || memberIds[0];
  } else {
    const primary = ranked[0].id;
    const prim = issueById.get(primary);
    memberIds = [primary, ...(prim.confused_with || [])];
    leadId = primary;
  }
