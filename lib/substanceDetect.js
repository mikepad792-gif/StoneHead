...
export function detectSubstance(message) {
  ...
  const distress = matches(toks, C_DISTRESS);
  if (distress.length) {
...
/**
 * Was the most recent assistant turn one of the fixed substance replies?
 *
 * The replies are fixed text, so identifying them is exact string comparison —
 * no state, no schema column, same trick derivePostCrisisState uses for the
 * crisis window. Used to append POST_SUBSTANCE_PROMPT for exactly one turn.
 *
 * @param {Array} history - [{ role, content }], chronological
 */
export function wasSubstanceTurn(history) {
  const assistants = (history || []).filter((m) => m && m.role === "assistant" && m.content);
  const last = assistants[assistants.length - 1];
  if (!last) return false;
  const text = String(last.content).trim();
  return text === SUBSTANCE_REPLY_S1.trim() || text === SUBSTANCE_REPLY_S2.trim();
}
