 ...
 * @param {boolean} [hasStrainName=false] - whether the message names a real
 *        strain (computed by the caller via strain retrieval). Used only to
 *        detect the cultivation-about-a-strain fork.
 * @returns {"STRAIN"|"CULTIVATION"|"CONSUMPTION-SAFETY"|"AMBIGUOUS"}
 */
export function classifyTopic(message, hasStrainName = false) {
  ...
  // 3. Cultivation vs the strain-named grow-trait fork.
  if (growSignal) {
    // "is Blue Dream hard to grow?" — a strain named in a grow-TRAIT question
    // (with no diagnosis symptom) genuinely forks grow-vs-hits → clarify.
    if (hasStrainName && growTrait && !diagnosis) return "AMBIGUOUS";
    return "CULTIVATION";
  }
