...
const CHALLENGE_CUES = [
  "pain", "anxiety", "dose", "dosage", "thc", "cbd", "thc:cbd",
...
// "mg" needs word boundaries — as a bare substring it fires inside "omg"
// (→ challenge at high confidence, which can even arm the Rumi beat).
const MG_RE = /\b\d+\s*mg\b|\bmg\b/;
...
export function detectFrame(message, history = []) {
  ...
  if (hasAny(text, CHALLENGE_CUES) || MG_RE.test(text)) {
...
const MENTAL_HEALTH_CUES = [
  ...
  // Over-consumption distress. "greening out"/"green out" miss the past
  // tense, and "took too much, I'm scared" matched nothing at all — the
  // person most in need of the safety route was classified STRAIN.
  "greened out", "took too much", "smoked too much", "too stoned",
];
...
const DIAGNOSIS_CUES = [
  ...
  // "sad" must be plant-anchored — bare "sad" routed "I'm sad today, what
  // should I smoke" into CULTIVATION with a random diagnosis block.
  // ("droop"/"droopy" already cover the common grower phrasing.)
  "damping off", "spider mite", "nute burn", "light burn", "droopy",
  "looks sad", "looking sad", "plant is sad", "she looks sad",
];
...
// ─── Vibe-tab handoff cues ──────────────────────────────────────────
// Both lists below are deliberately NARROW — on the vibe tab a false positive
// fires a handoff button on an ordinary philosophy message and breaks the
// tab. Prefer missing a case over over-firing: a missed grow/strain question
// is a small leak; a handoff on "what's the point of anything" is a disaster.

// Explicit strain-recommendation / product intent.
const STRAIN_REC_CUES = [
  "what strain", "which strain", "recommend a strain", "strain for",
  "what should i smoke", "best strain", "good strain", "any strain",
  "strain rec", "indica or sativa", "something to help me sleep",
  "something for energy",
];

// Explicit grow-question intent. classifyTopic's CULTIVATION cues are NOT
// safe here: they were tuned for the plant tab, where "burn", "bug", "dying"
// and " grow " always mean a plant. On the vibe tab those same substrings
// live inside "burned out", "it's been bugging me", "part of me is dying",
// and "growing as a person" — every one of them a core philosophy message.
const GROW_QUESTION_CUES = [
  "my plant", "my plants", "my grow", "my leaves", "my seedling",
  "leaves are", "leaves turning", "leaves look", "yellowing",
  "how to grow", "how do i grow", "hard to grow", "easy to grow",
  "grow tent", "grow light", "grow room", "growing weed", "growing cannabis",
  "grow weed", "grow cannabis", "in veg", "veg stage", "flowering time",
  "flowering stage", "germinating", "seedling", "nutes", "overwatering",
  "trichome", "bud rot", "root rot", "spider mites", "powdery mildew",
  "when to harvest", "ready to harvest",
];

/**
 * TRUE only when the message asks for a strain/product recommendation.
 * Does NOT use classifyTopic's "STRAIN" return — that is a catch-all default
 * that matches all unclassified text, including every philosophy message.
 */
export function isStrainRecRequest(message) {
  const text = " " + String(message || "").toLowerCase() + " ";
  return hasAny(text, STRAIN_REC_CUES);
}

/**
 * TRUE only when the message is unmistakably about growing a cannabis plant.
 * Does NOT use classifyTopic's "CULTIVATION" return — see the cue-list note
 * above for why that return value over-fires on vibe-tab text.
 */
export function isGrowQuestion(message) {
  const text = " " + String(message || "").toLowerCase() + " ";
  return hasAny(text, GROW_QUESTION_CUES);
}

/**
 * Route a vibe-tab turn. Safety is checked FIRST so a message that trips
 * both ("I took too much of that strain") gets helped where they stand,
 * never redirected. The safety branch is not a handoff and must never
 * become one — the two share this router and nothing else.
 *
 * @returns {"SAFETY"|"HANDOFF"|"NONE"} SAFETY = answer in place with the
 *   consumption-safety prompt; HANDOFF = point to Talk the Plant;
 *   NONE = ordinary vibe turn, untouched.
 */
export function routeVibeTurn(message) {
  if (classifyTopic(message) === "CONSUMPTION-SAFETY") return "SAFETY";
  if (isGrowQuestion(message) || isStrainRecRequest(message)) return "HANDOFF";
  return "NONE";
}
