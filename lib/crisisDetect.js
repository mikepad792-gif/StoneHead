...
/**
 * CUES THAT ONLY FIRE ON A SHORT TURN.
 *
 * "me" and "everything" are the two broadest entries in the set — as bare
 * words they appear in an enormous amount of ordinary English, and inside a
 * window they were promoting sentences that plainly meant something else:
 *
 *   "he gave me a ride"          -> tier 2
 *   "my sister called me back"   -> tier 2
 *   "that made me feel alive"    -> tier 2   (the opposite of ideation)
 *
 * They are worth keeping, because "me." and "everything." ARE the answer
 * sometimes. What makes them meaningful is that they arrive as a terse answer
 * to a direct question — which is the spec's own reasoning for why this window
 * works at all: answers to direct questions are short and predictable.
 *
 * So they keep their coverage of the answer shape and lose their coverage of
 * every sentence that happens to contain a pronoun.
 */
export const POST_CRISIS_CUES_SHORT = ["me", "everything"];
...
/**
 * POLARITY GUARD FOR THE ALIVE FAMILY.
 *
 * "alive" is a bare word whose meaning here is entirely carried by what sits
 * next to it. Inside a window, without this, every one of these promotes to
 * tier 2:
 *
 *   "I feel alive"  "I feel alive again"  "I've never felt more alive"
 *   "glad to be alive"  "first time in months I feel alive"
 *
 * Those are the OPPOSITE of the thing being detected, and firing a crisis
 * response at somebody who just said they feel alive is its own small harm.
 *
 * Two rules, in order:
 *
 *   1. An INTENSIFIER wins outright. "more alive", "so alive", "alive again"
 *      are positive regardless of anything else in the sentence — which is
 *      what makes "I've never felt more alive" come out right despite the
 *      "never".
 *
 *   2. Otherwise a FEELING VERB near the word reads positive, unless it is
 *      negated. "I feel alive" is positive; "I don't feel alive" is not, and
 *      that one should still fire.
 */
const ALIVE_INTENSIFIERS = ["more alive", "so alive", "alive again", "most alive"];
const ALIVE_POSITIVE_VERBS = ["feel", "feels", "feeling", "felt", "glad", "happy", "lucky", "grateful", "alive"];
const ALIVE_NEGATIONS = ["not", "dont", "doesnt", "cant", "never", "barely", "hardly", "stopped", "wont"];
const ALIVE_LOOKBACK = 3;

function aliveReadsPositive(rawMessage, toks) {
  const flatText = toks.join(" ");
  if (ALIVE_INTENSIFIERS.some((p) => flatText.includes(p))) return true;

  const i = toks.indexOf("alive");
  if (i === -1) return false;

  const start = Math.max(0, i - ALIVE_LOOKBACK);
  const before = toks.slice(start, i);
  const hasVerb = before.some((t) => ALIVE_POSITIVE_VERBS.includes(t));
  if (!hasVerb) return false;

  // "I don't feel alive" — the feeling verb is there but negated, so this is
  // the concerning reading after all.
  return !before.some((t) => ALIVE_NEGATIONS.includes(t));
}
...
const C_POST_CRISIS_SHORT = compile(POST_CRISIS_CUES_SHORT);
...
/**
 * Has ANY user turn in this thread ever fired the crisis layer?
 *
 * Pure, derived from history, no schema column — same trick as the window.
 * Used by the postwork function to decide whether a thread may be titled at
 * all: postwork is skipped ON a safety turn, but a thread that escalates on
 * turn 3 was already titled on turn 1, and any later ordinary turn would
 * otherwise regenerate a title from a transcript that now contains the crisis.
 *
 * @param {Array} history - [{ role, content }], chronological
 */
export function threadEverFired(history) {
  const raw = history || [];
  let state = { inWindow: false, turnsOpen: 0, anchorTier: 0 };
  for (let i = 0; i < raw.length; i++) {
    const m = raw[i];
    if (!m || m.role !== "user" || m.content == null) continue;
    const effective = resolveTurn(m.content, historyBefore(raw, i), state);
...
function resolveTurn(message, history, state) {
  ...
  const aliveFamily = aliveReadsPositive(message, toks)
    ? [] // "I feel alive" is the opposite of the thing being detected.
    : matches(toks, C_POST_CRISIS_EXACT, false);
  const shortAnswer = wordCount(message) <= SHORT_ANSWER_MAX_WORDS
    ? matches(toks, C_POST_CRISIS_SHORT, false)
    : [];
  const answered = [
...
// ─── The tier-1 system-prompt block: MOVED (Addendum C4) ────────────
//
// CRISIS_CLARIFY_PROMPT used to live here and was appended by chat-send.js
// under `crisis.tier === 1 && !safetyMode`. After Addendum B routed every
// tier >= 1 into safety mode, that condition could never be true, so the
// clarify instruction silently stopped shipping and tier 1 inherited the
// tier-2 stance — which is how "i don't want to be here anymore" started
// returning "I need you to stay. Right now, in this moment, with me."
//
// It now lives in prompts/crisis.js as CLARIFY_FIRST, appended by
// buildCrisisPrompt(kind, { clarify: true }). Same words, one file, and the
// tier that selects it is the tier that builds the prompt — there is no
// separate condition left to fall out of sync.
//
// Deleted rather than deprecated on purpose: a second copy of a safety
// instruction that nothing imports is exactly the thing that just failed.
