...
function loadDabs() {
  ...
  const filePath = path.join(__dirname, "../data/dabs_knowledge.json");
  dabCache = JSON.parse(fs.readFileSync(filePath, "utf-8"));
  return dabCache;
}

function loadSlang() {
  if (slangCache) return slangCache;
  const filePath = path.join(__dirname, "../data/slang_dictionary.json");
  slangCache = JSON.parse(fs.readFileSync(filePath, "utf-8"));
  return slangCache;
}
