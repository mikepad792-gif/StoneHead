...
import { resolveStrainName, resolveExactName, normalizeStrainName } from "./strainSearch.js";
...
// Tokens that should never appear in a real strain name. If the cleaned
// candidate still contains any of these, it's intent scaffolding / a sentence
// fragment ("you didn't save it"), not a strain — write nothing.
const REJECT_TOKENS = new Set([
  "you", "your", "yours", "u", "it", "its", "me", "my", "mine", "we", "us",
  "they", "them", "i", "im", "ive", "id", "ill",
  "save", "saved", "saving", "saves", "remember", "remembered", "forget", "forgot",
  "did", "didn", "didnt", "dont", "don", "doesn", "doesnt", "not", "no", "never",
  "what", "why", "how", "who", "when", "where", "is", "was", "are", "were", "be",
  "anything", "nothing", "something", "everything", "that", "this", "these", "those",
  "please", "thanks", "thank", "ok", "okay", "yeah", "yes", "nah", "cool",
]);
...
function extractStrainPhrase(message) {
  ...
  // Strip trailing "as a liked strain / as a fav / to my favorites / ..." tails.
  t = t.replace(
    /\s+(?:as|to|in|on)\s+(?:a\s+|my\s+|the\s+)?(?:liked(?:\s+strain)?|saved|fav|favs|fave|faves|favorite|favourite|favorites|favourites|list)\b.*$/,
    ""
  ).trim();
  // Strip trailing "for me / for later / for now".
  t = t.replace(/\s+for\s+(?:me|later|now)\b.*$/, "").trim();
...
// Uppercase the first letter of each word — but only at real word starts
// (start of string, after a space or hyphen). The old \b\w version treated
// an apostrophe as a boundary, mangling "charlotte's" into "Charlotte'S".
function titleCase(s) {
  return s.replace(/(^|[\s-])([a-z0-9])/g, (m, lead, c) => lead + c.toUpperCase());
}

// Tokens that never start/appear inside a strain n-gram worth scanning —
// stops the embedded exact-name scan from matching through sentence glue
// (REJECT_TOKENS) or article noise ("the white" hitting a "The-…" name
// inside an unrelated sentence).
const SCAN_SKIP = new Set(["the", "a", "an", "my", "of", "and", "or", "for", "to", "in", "on", "as", "strain", "liked", "favorite", "favourite", "fav", "list"]);
...
export function detectSaveIntent(message) {
  ...
  // 2.5. Embedded exact-name scan over the WHOLE message. This is what
  //    rescues a save buried in sentence scaffolding ("bro you didn't save
  //    blue dream for me") — without it, the guards below correctly reject
  //    the fragment but the real strain in it gets dropped too. Exact
  //    normalized equality only (no fuzz), longest n-gram first, skipping
  //    grams that contain sentence glue.
  if (!resolved) {
    const msgTokens = normalizeStrainName(message).split(" ").filter(Boolean);
    outer: for (let n = Math.min(4, msgTokens.length); n >= 2; n--) {
      for (let i = 0; i + n <= msgTokens.length; i++) {
        const gram = msgTokens.slice(i, i + n);
        if (gram.some((t) => REJECT_TOKENS.has(t) || SCAN_SKIP.has(t))) continue;
        const hit = resolveExactName(gram.join(" "));
        if (hit) {
          resolved = hit;
          break outer;
        }
  ...
  //    null type. Guards keep sentence fragments / non-strains out: if the
  //    cleaned phrase still holds intent scaffolding ("you didn't save it"),
  //    write nothing rather than store garbage. Guards run on the NORMALIZED
  //    phrase; the stored name comes from the raw phrase so apostrophes
  //    survive ("Charlotte's Web", never "Charlotte S Web").
  ...
  if (
    ...
    tokens.some((t) => REJECT_TOKENS.has(t))
  ) {
    return null;
  }
  ...
  return {
    type: "liked_strain",
    value: { strain_name: titleCase(phrase), strain_type: null },
    frame: null,
  };
}
