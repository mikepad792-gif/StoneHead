...
export function searchStrains(userMessage) {
  ...
  const scored = strains
    .map((strain) => {
      ...
      // the full name as a consecutive run of whole words ("Blue Dream",
      // "OG Kush") scores highest; "Goo" no longer matches inside "good".
      const nameTokens = strain._nameTokens;
      if (nameTokens.length > 0 && hasConsecutive(msgTokens, nameTokens)) {
        score += 5;
        nameHit = true;
      } else {
        // Fall back to whole-token name matching (a message word equal to a
        // name token), still word-boundary — not substring.
        for (const tok of nameTokens) {
          if (tok.length > 3 && msgTokenSet.has(tok)) {
            score += 3;
            nameHit = true;
          }
        }
